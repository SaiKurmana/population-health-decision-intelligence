...
def get_agent_arn(region):
    ...
    try:
        ...
        response = ssm_client.get_parameter(Name='/workshop/pop_health_mcp/agent_arn')
...
def update_mcp_config(url):
    ...
    try:
        ...
        if 'pop_health_remote' in config.get('mcpServers', {}):
            config['mcpServers']['pop_health_remote']['url'] = url
            config['mcpServers']['pop_health_remote']['enabled'] = True
            ...
            print("❌ pop_health_remote not found in mcp.json")
            return False
